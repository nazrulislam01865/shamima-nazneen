...
@foreach($providedLinks as $profileLink)
    @php
    ...
    @endphp
    <a class="social-link social-link-{{ $profileClass }} {{ $profileClass === 'chorki' ? 'chorki' : '' }}" href="{{ $profileUrl }}" @if($profileIsExternal) target="_blank" rel="noopener noreferrer" @endif aria-label="{{ $profileLabel }}">
        <span class="social-icon {{ $profileIconUrl ? 'has-custom-icon' : '' }}" aria-hidden="true">
            @if($profileIconUrl)
